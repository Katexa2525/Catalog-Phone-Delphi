...
var
  ...
  Ptr, PrtPrev, PrtNext: PCatalog; // промежуточные переменные для ссылок на элементы
begin
  ...
  Begin
    try
      New(Ptr); //Иначе может появиться ошибка Access violation
    except
      Result:=False;
      Exit;
    end;
    ...
    // обмен через промежуточную переменную Ptr
    Ptr^.Prev:=El1^.Prev;
    Ptr^.Next:=El1^.Next;
  ...
  Begin
    ...
    begin
      if Ptr_1.Data.Price < PtrBestValue.Data.Price then
        PtrBestValue := Ptr_1; // значение наименьшего элемента
      // к след элементу
      Ptr_1:=Ptr_1^.Next;
    end;
...
begin
  ...
  if ACol > 0 then begin // если не нулевая колонка
   ...
   SetColumnHeaders(StringGrid_Catalog); // удаляю все сортировки
  // сортировка
  SortItems;
  //
